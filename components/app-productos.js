(() => {
  ...
  class AppProductos extends HTMLElement {
    ...
    render() {
      ...
      if (!(type == "wrap" || type == "oneline")) {
        type = "wrap";
        this.setAttribute("type", type);
      }

      if (type === "cart" && localStorage.getItem("hasAcceptedCookies")) {
        this.handleCartRendering();
      }
    }

    async handleCartRendering() {
      try {
        const response = await fetch('/PMD-Brisella/scripts/json/productList.json');
